/*
 * FileName: UninformedSearch.aspx.cs
 * This page contains the Algorithm Implementation for Uninformed Search WIthout Congestion
 * 
 * */
...
public partial class UnInformedSearch : System.Web.UI.Page
{
    ...
    {
        //UnInformedSearch u = new UnInformedSearch();
        hfSource.Value = "";
        getValues();
    }
    ...
    public void getValues()
    {
        ...
        if (hfSource.Value != "")
        {
            /* The start time of user's search is stored in the hidden field - hfTime */
            if(hfTime.Value != "")
            {
        ...
        {
            /* Default testing condition - this is not used */
        ...
        if(hfTime.Value == "")
        {
            dateTime1 = Convert.ToDateTime("2012-04-08 17:38:00"); 
        }
        else
        {
        ...
        while (true)
        {
            ...
            /* The below step finds the neighbors of the node under considerarion  */
            ...
            if (dsNeighbors.Tables[0].Rows.Count > 0)
            {
                ...
                /* The below loop is used to find the distance between the neighbors of the current intersection and the initial location of the user */
                ...
                {
                    /* Each of the neighbors are considered in ascending order of their distance from the initial location */
                    while (neighborCount < dsDistances.Tables[0].Rows.Count)
                    {
                        ...
                        /* visitedNeighbors arraylist contains the list of neighbors taht are already visited. The intersection under
                         * consideration is added to the list only if the node is not present in the visitedIntersections arraylist */
                ...
                string returnValue = bllObject.checkOperationalWithoutCongestion(sourceIntersectionLat, sourceIntersectionLng, destIntersectionLat, destIntersectionLng, datetime, addedTime);
                
                if (Convert.ToInt32(returnValue) > 0)
                {
                    hfReturnValue.Value = "1";
                    DataSet dsBlockLocation = new DataSet();
                    /* The below method call checks if there any available slots in the block id which was returned */
                    dsBlockLocation = bllObject.findBlockStartLocation(Convert.ToInt32(returnValue));
                    if (dsBlockLocation.Tables[0].Rows.Count > 0)
                    {
                        /* If an available parking block is found, the user has found the parking and the algorithm terminates by displaying the final route */
    ...
    {
        getDataFromExcel();
        Response.Redirect("TestResultsUninformedSearch.aspx");
    }
    ...
    void getDataFromExcel()
    {
        ...
        bllObject.deleteTestData("WITHOUT");
        int countv = 0;
        using (OleDbConnection connection = new OleDbConnection(con))
        {
            connection.Open();
            OleDbCommand command = new OleDbCommand("select * from [TestData$]", connection);
            // Data from the spreadsheet is read and the getValues() function is called for each input row in the spreadsheet
            using (OleDbDataReader dr = command.ExecuteReader())
            {
                while (dr.Read())
                {
                    ...
                    bllObject.insertTestData(Convert.ToDecimal(dr[1]), Convert.ToDecimal(dr[2]),Convert.ToDecimal(hfDestination.Value.Split(',')[0]) , Convert.ToDecimal(hfDestination.Value.Split(',')[1]) ,dr[3].ToString(), time,travelDist,travelTime);
                    countv++;
